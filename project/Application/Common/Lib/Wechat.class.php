...
class Wechat
{
    ...
    const NATIVE_URL = 'weixin://wxpay/bizpayurl?';
    const MCH_API_URL_PREFIX = 'https://api.mch.weixin.qq.com';
    const MCH_SHORT_URL = '/tools/shorturl';
    ...
    private function log($log){
    	if ($this->debug ) {
            $logfile = RUNTIME_PATH . 'Logs/wechat-' . date('Ymd') . '.log';
            error_log($log . "\n", 3, $logfile);
	...
	*/
	...
	public function checkAuth($appid='',$appsecret=''){
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 1800;
	...
	*/
	...
	public function getPaySignature($arrdata) {
        ...
        \Common\Lib\Utils::log('wechat', 'wechat.log', "sign string is: " . $paramstring . '&key=' . $this->paysignkey);
		$paySign = strtoupper(md5($paramstring . '&key=' . $this->paysignkey));
		return $paySign;
	}
     ...
     *
     */
    public function getNativeUrl($productId, $skuId) {
        $nonceStr = $this->generateNonceStr(32);
		$arrdata = array("appid" => $this->appid, "mch_id" => $this->mchid, "product_id" => $productId . '_' . $skuId, "time_stamp" => time(), "nonce_str" => $nonceStr);
		$paySign = $this->getPaySignature($arrdata);
        $arrdata['sign'] = $paySign;
        $nativeUrl = self::NATIVE_URL . http_build_query($arrdata);
		return $nativeUrl;
    }

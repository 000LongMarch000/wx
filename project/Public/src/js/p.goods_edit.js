define(function(require, exports, module){
  ...
  var Skus = {
    ...
    createItem: function(data){
      ...
      $item.find('input[name="sku_price[]"]').rules('add', {
        number: true,
        min: 0
      });
  ...
  var TagEditor = {
    init: function(tags){
      ...
      var $tagsinput = $el.find('.bootstrap-tagsinput');

      $input.tagsinput('input')
        .on('focus', function () {
          $tagsinput.addClass('focus-in');
        })
        .on('blur', function () {
          $tagsinput.removeClass('focus-in');
          var $this = $(this),
              tag   = $this.val();

          /* 源码 libs/bootstrap-tagsinput.js 第149行有修正
           * @从2.3.2开始
           */

          if (tag.length > 0 && $.inArray(tag, $input.tagsinput('items')) < 0) {
            $input.tagsinput('add', tag);
          }
        });

      $el.on('click', '.J_tags a', function () {
        $input.tagsinput('add', $(this).html());
      }).on('mousedown', '.tag', function (e) {
        e.preventDefault();
      });
  ...
  var Editor = {
    init: function(opts){
      ...
      if (!this.opts.shop_id) throw new Error('shop_id required');
      if (!this.opts.id) this.status = 'new';
      else this.status = 'edit';

      Skus.init();
      Imgs.init();
      TagEditor.init(this.opts.tags);
      this.render().bindEvents();
    ...
    render: function(){
      ...
      //销售方式的初始化，模板中逻辑不太好判定，就写在这里了
      var $form = $('#J_goodsForm');
      var salesType=  $form.find('[name=sales_type]') .val().split(',');

      if(salesType.length == 1){
        $form.find('input[type=checkbox][value='+salesType[0]+']').icheck('checked').icheck('disabled');

        if(salesType[0] == 'online') $('#J_postageSec').show();
        else $('#J_postageSec').hide();
      }else{
        $.each(salesType, function(i, v){
          $form.find('input[type=checkbox][value='+v+']').icheck('checked');
        })
      }

      //$('#J_commission').number(1, '');
